...
differentiation_str = "EneroComprimido"
...
#Método encargado de asignar el actor comprimido que se va a ajustar realizando unas cuantas epocas de entrenamiento
def obtener_actor_comprimido():
    ...
    checkpoint.restore("../DECORE/ApagadoAleatorio/apagado_30%")
    #Aplico la compresion final
    actor_final = ModeloBaseComprimido(actor_mascaras,tiene_agentes=False)
    actor_final.build()
    return actor_final

#Busca en el archivo de logs obtenido tras el entrenamiento del modelo base el mejor crítico encontrado
def obtener_mejor_critico(log_antiguo):
    with open(log_antiguo) as fp:
        for line in reversed(list(fp)):
            arrayLine = line.split(":")
            if arrayLine[0] == 'MAX REWD':
                model_id = (arrayLine[2].split(",")[0]).strip()
...
class PPOActorCritic:
    def __init__(self):
        ...
        self.actor = obtener_actor_comprimido()
...
if __name__ == "__main__":
    ...
    if args.i==0:
        #Se va a comenzar a procesar el primer lote de X episodios --> Restaurar el estado del mejor crítico que ya tenemos guardado
        log_antiguo = "./Logs/expEnero_3top_15_B_NEWLogs.txt"
        critico_id = obtener_mejor_critico(log_antiguo)
        ruta_check = "./modelsEnero_3top_15_B_NEW/ckpt_CRT-" + critico_id
        checkpoint = tf.train.Checkpoint(model=agent.critic)
        checkpoint.restore(ruta_check).expect_partial()
        print("Se ha restaurado con éxito el mejor crítico obtenido en el proceso de entrenamiento antiguo")
    ...
    for iters in range(args.e): #Iteraciones del episodio 'e'
        states = []
        critic_features = []
        tensors = []
        actions = []
